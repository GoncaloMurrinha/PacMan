...
public class pmanc extends JFrame {
	...
	public pmanc() {
		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
					if (!paused) {
						paused = true;
						Pause pauseWindow = new Pause(pmanc.this);
				...
				if (start=true) {   
					 if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
							paused = !paused;
							return; // N?o processa outros comandos quando ESC ? pressionado
						}
		...
		class Aux extends TimerTask{
			...
			@Override
			public void run() {
			    if (paused) return; // Adicione esta linha para pausar o jogo
				secor.setText(""+score);
